...
@section('page-title', 'Edytuj projekt')
@section('breadcrumb', 'Zarządzanie projektami → Edycja')
        ...
        <form method="POST" action="{{ route('admin.projects.update', $project) }}" class="space-y-6">
            ...
            <div>
                ...
                <textarea id="description" name="description" rows="3"
                          class="w-full px-4 py-2.5 bg-gray-800 border @error('description') border-red-500 @else border-gray-700 @enderror rounded-lg text-white focus:outline-none focus:ring-2 focus:ring-brand-500">{{ old('description', $project->description) }}</textarea>
                @error('description') <p class="mt-1 text-xs text-red-400">{{ $message }}</p> @enderror
            </div>

            <div class="p-4 rounded-lg bg-gray-800/50 border border-gray-700 space-y-4">
                ...
                <div>
                    ...
                    <input type="password" id="github_token" name="github_token" value="{{ old('github_token', $project->github_token) }}"
                           class="w-full px-4 py-2.5 bg-gray-800 border @error('github_token') border-red-500 @else border-gray-700 @enderror rounded-lg text-white focus:outline-none focus:ring-2 focus:ring-brand-500" placeholder="Zostaw puste aby nie zmieniać (jeśli już ustawiono)">
                    @error('github_token') <p class="mt-1 text-xs text-red-400">{{ $message }}</p> @enderror
                </div>

                <div>
                    <label for="github_repo" class="block text-sm font-medium text-gray-300 mb-1.5">Pełna nazwa repozytorium (np. uzytkownik/repo)</label>
                    <input type="text" id="github_repo" name="github_repo" value="{{ old('github_repo', $project->github_repo) }}"
